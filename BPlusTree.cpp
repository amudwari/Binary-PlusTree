...
void BPlusTree::insert(KeyType aKey, std::string aValue)
{
    if (isEmpty()) {
...
void BPlusTree::startNewTree(KeyType aKey, std::string aValue) {
    LeafNode* newLeafNode = new LeafNode(fOrder);
    newLeafNode->createAndInsertRecord(aKey, aValue);
    fRoot = newLeafNode;
}

void BPlusTree::insertIntoLeaf(KeyType aKey, std::string aValue)
{
...
std::vector<BPlusTree::EntryType> BPlusTree::range(KeyType aStart, KeyType aEnd)
{
    ...
    std::vector<std::tuple<KeyType, std::string, LeafNode*>> entries;
